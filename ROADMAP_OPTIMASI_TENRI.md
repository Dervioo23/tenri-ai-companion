...
| 1. Silero VAD | Root cause "VAD budek" ditemukan & diperbaiki; menunggu konfirmasi mic live | Kelumpuhan Silero di perangkat ini ternyata **bukan** ketidakstabilan perangkat melainkan bug nyata: model v5 memerlukan 64 sampel context dari frame sebelumnya (input 576, bukan 512); tanpa itu probabilitas ~0,0 untuk semua frame termasuk suara keras-jelas, sehingga endpointer tak pernah membuka. Diperbaiki di `SileroVAD.probability`, diverifikasi pada sampel suara nyata (prob 1,000; hening 0,009), dan dikunci dua lapis regresi: unit (input wajib 576 + context carry) dan end-to-end korpus audio (`silero-context-speech.wav` nyata → endpointer wajib memicu). `start()` kini juga menunggu frame audio pertama dengan timeout dan auto-fallback ke `BackgroundListener` bila mic bisu. `.env` dikembalikan ke `SILERO_VAD_ENABLED=true`. Belum 100 persen sampai satu rehearsal mic live mengonfirmasi `[VAD-DEBUG] max_prob` menyeberang ambang dan capture muncul. |
...
| 4. Pipeline event-driven | Milestone 4A-4B selesai, migrasi parsial | Capture sudah dipisahkan menjadi `CaptureStage`; klasifikasi dan keputusan dasar berada di `RouterStage`; query rewriting, retrieval, grounding context, dan pembangunan prompt berada di `BrainStage`. Event bus bertipe, trace JSONL tanpa transkrip, dan korpus audio regresi juga aktif. TTS dan Player masih perlu dipisahkan sebelum barge-in layak diaktifkan. |
...
Keputusan operasional saat ini: pertama, konfirmasi Silero VAD secara live (jalankan dengan `SILERO_VAD_ENABLED=true` dan `SILERO_VAD_DEBUG=true`, panggil "Tenri", pastikan `[VAD-DEBUG] max_prob` menyeberang `start_prob` dan capture muncul) — baru setelah itu boleh memangkas filter halusinasi/RMS/durasi yang menjadi mubazir. Kedua, jalankan rehearsal baru dengan `ELEVENLABS_STREAMING=true`, kumpulkan sedikitnya 20 turn, lalu bandingkan p50/p95 baru. Milestone 4A-4B membangun batas Capture, Router, dan Brain tanpa mengubah concurrency audio. Migrasi berikutnya adalah memisahkan TTS dan Player menjadi stage yang dapat dibatalkan; barge-in tetap menunggu jalur playback tersebut dan strategi echo yang terbukti.
